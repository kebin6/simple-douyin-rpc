...
import (
	"encoding/json"
	"fmt"
	"github.com/kebin6/simple-douyin-rpc/open/context"
...
func (user *User) GetUserInfo(openid string) (userInfo *Info, err error) {
	...
	if result.Data.ErrCode != 0 {
		err = util.NewCodeDouYinError("GetUserInfo", result.Data.DYError, nil)
		return
	}
...
func (user *User) ListFans(openid string, cursor, count int64) (fans *ListInfo, err error) {
	...
	if result.Data.ErrCode != 0 {
		err = util.NewCodeDouYinError("ListFans", result.Data.DYError, nil)
		return
	}
	fans = &result.Data
	return
